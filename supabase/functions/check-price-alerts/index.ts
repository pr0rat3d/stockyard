...
serve(async (req) => {
  ...
  // `prices` now carries both categories in one snapshot — check alerts against each.
  const checks = [
  ...
  for (const { category, currentPrice } of checks) {
    ...
    for (const alert of alerts) {
      ...
      triggered.push({ alert_id: alert.id, category, user: user.email, price: currentPrice });
    }
  }
